...
<xsl:stylesheet xmlns="http://www.icpsr.umich.edu/DDI" xmlns:dc="http://purl.org/dc/elements/1.1/"
  ...
  <!-- create variable groups based on concepts -->
  <xsl:param name="createVarGoup">true</xsl:param>

  <!-- create default system missing statement for variables -->
  <xsl:param name="createDefaultSystemMissing">true</xsl:param>

  <!-- prefix variable label with variable name -->
  <xsl:param name="prefixVarLableWithVarname">true</xsl:param>
  ...
  <xsl:template match="s:StudyUnit">
    <codeBook>
      ...
      <dataDscr>
        <xsl:choose>
          <xsl:when test="$createVarGoup='true'">
            <xsl:call-template name="variableGroup"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="oneVariableGroup"/>
          </xsl:otherwise>
        </xsl:choose>
  ...
  <xsl:template match="s:Abstract">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="r:Content/@xml:lang=$lang">
            <abstract>
              <xsl:attribute name="xml-lang">
                <xsl:value-of select="r:Content/@xml:lang"/>
              </xsl:attribute>
              <xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text>
              <xsl:apply-templates select="r:Content[@xml:lang=$lang]/*"/>
              <xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
            </abstract>
          </xsl:when>
          <!--xsl:otherwise>
  ...
  <xsl:template name="bibCitation">
    ...
    <biblCit>
      <xsl:value-of select="normalize-space($bibCitation)"/>
    </biblCit>
  </xsl:template>
  ...
  <xsl:template name="variableGroup">
    ...
    <xsl:variable name="nonConcept">
      <xsl:for-each select="*//l:Variable">
        <xsl:choose>
          <xsl:when test="l:ConceptReference"/>
          <xsl:otherwise>
            <xsl:value-of select="l:VariableName"/>
            <xsl:text> </xsl:text>
          </xsl:otherwise>
    ...
    <varGrp ID="VG0" type="subject" var="">
      <xsl:attribute name="var">
        <xsl:value-of select="normalize-space($nonConcept)"/>
      </xsl:attribute>
      <labl>
        <xsl:value-of select="util:i18n('defaultVarGroupName')"/>
      </labl>
    </varGrp>
  </xsl:template>

  <xsl:template name="oneVariableGroup">
    ...
    <varGrp ID="VG0" type="subject" var="">
      ...
      <labl>
        <xsl:value-of select="util:i18n('variables')"/>
      </labl>
    </varGrp>
  </xsl:template>
  ...
  <xsl:template match="l:Variable">
    ...
    <var>
      ...
      <xsl:variable name="width"
        select="../../../pd:PhysicalDataProduct/pd:RecordLayoutScheme/pd:RecordLayout/pd:DataItem[pd:VariableReference/r:ID=$varID]/pd:PhysicalLocation/pd:Width"/>

      <location>
        <xsl:attribute name="width">
          <xsl:choose>
            <xsl:when test="l:Representation/l:DateTimeRepresentation">
              <xsl:text>8</xsl:text>
            </xsl:when>
            <!--xsl:when test="l:Representation/l:TextRepresentation">
      ...
      <!-- label -->
      <labl>
        <xsl:if test="$prefixVarLableWithVarname='true'">
          <xsl:value-of select="l:VariableName"/>
          <xsl:text> </xsl:text>
        </xsl:if>
        <xsl:choose>
      ...
      <xsl:if
        test="(l:Representation/l:NumericRepresentation or l:Representation/l:CodeRepresentation) and $createDefaultSystemMissing='true'">
        <catgry missing="Y">
